...
```text
Where can we get renewable-energy-specific financial reference numbers?
Which sources have open numbers?
Which sources have gated numbers?
Which sources are merely pathways?
Which hazard × asset pairs are strong enough for a deep dive?
```
...
range-validation-hazard-modeling/
  ...
  01_pairs/
    solar_hail/
      README.md                              <-- first deep-dive pair dossier
      benchmark_number_matrix.csv
      source_matrix.csv
  ...
  data/
    ...
    solar_hail_value_damage_crosswalk.csv/json
    solar_hail_damage_curve_intensity_reference.csv/json
    source_registry_solar_hail.csv
    source_registry_solar_hail.json
  02_crosswalks/
    README.md
    solar_hail_value_damage_crosswalk.md       <-- value/damage denominator crosswalk
  99_metadata/
    bibliography.md
    manifest.md
```
...
## v0.2 addition — value and damage-model cross-reference

The package now cross-references the solar-hail reference numbers against the damage-modeling value workbook and canonical hail damage artifact.

```text
02_crosswalks/solar_hail_value_damage_crosswalk.md
```

This is intentionally a **normalization layer**, not a calibration rule. It adds value because solar-hail benchmark numbers often arrive as gross claim dollars per MW, while the M3 hail damage curve damages a PV_MODULE / PV_ARRAY failure-unit. The crosswalk normalizes benchmark rows to:

```text
installed TIV
physical replaceable value
PV_ARRAY module hardware value
PV_ARRAY + replacement fieldwork value
extended PV_ARRAY + mounting + fieldwork value
```
...
The flood crosswalk is `RESOLVED_FROM_DAMAGE_MODELING` (Case A): it is cross-mapped onto the **canonical `flood_solar` cell** in [`Damage_Modeling`](https://github.com/aamani-ai/Damage_Modeling). The flood failure grain is the **8 named canonical failure units** (`FS_INV`, `FS_SWG`, `FS_XFMR`, `FS_COMB`, `FS_SCADA`, `FS_CABLE`, `FS_FOUND`, `FS_PVMOD` — mostly electrical, not PV-module glass like hail). Benchmark rows are normalized to installed TIV (`$1,120,000/MWdc`), physical replaceable value (`$877,796/MWdc`), and the canonical failure-unit buckets: the **primary depth-driven electrical** sum (INV+SWG+XFMR+COMB+SCADA) `≈$146,947/MWdc` (13.12% TIV) and the **all-8-unit envelope** `≈$538,607/MWdc` (48.09% TIV). Depth→DR ordinates are the real piecewise-linear curves from the canonical `flood_solar` curve artifact (public-source-anchored engineering parameterizations: DOE/FEMP, NEMA, FEMA, USACE HEC-FIA), not a generic-industrial proxy.

> **Correction note (v0.3.1).** The v0.3 build shipped a single invented `FLOOD_ELECTRICAL_BOS` bucket (~`$217,279/MWdc`, 19.4% TIV) and marked the crosswalk `PENDING_UPSTREAM_ARTIFACTS`, mistakenly believing no solar-flood damage artifact existed. The canonical `flood_solar` cell **does** exist; the mechanism (electrical/BOS/foundation inundation) was right but the grain, IDs, and curve were non-canonical. v0.3.1 replaces them with the canonical 8 failure units + real curve and lifts the crosswalk out of PENDING.
...
## v0.5 addition — wind_convective_wind pair dossier (onshore turbines; convective sub-perils)

The package now includes its first **wind-asset** pair dossier: [`01_pairs/wind_convective_wind`](01_pairs/wind_convective_wind/README.md), plus a value/damage crosswalk at [`02_crosswalks/wind_convective_wind_value_damage_crosswalk.md`](02_crosswalks/wind_convective_wind_value_damage_crosswalk.md).

**Scope fence (load-bearing).** This pair covers onshore wind turbines under **CONVECTIVE wind only**: tornado [T] and straight-line/derecho [W]. The hazard axis is the dimensionless speed ratio `X_SPEED_RATIO_TO_IEC` (`r = V_3s_hub / Ve50_class`). Tornado is the **D50-shifted** pathway plus a `0.10` core-exposure fraction; straight-line/derecho is the **SLW (D50-baseline)** pathway of the **same** canonical curves (there is no separate strong-wind-for-wind cell upstream). **Hurricane/TC** and **hail-dominant SCS** are named-and-quarantined (`HURRICANE_TC`, `HAIL_CONTAMINATED_SCS`) so wrong-peril numbers (e.g. Moody's hurricane-contaminated "US wind PML +58%") are never averaged into a convective-wind figure.

The wind crosswalk is `RESOLVED_FROM_DAMAGE_MODELING`: the **canonical, PROMOTED `wind_tornado_wind` cell** exists in [`Damage_Modeling`](https://github.com/aamani-ai/Damage_Modeling) (`docs/cells/wind_tornado_wind/current/`, `canonical_runtime_artifact=true`), so the **5 named WT_* structural failure units** (blade, tower, nacelle, foundation, power-electronics), their value shares, AND the real speed-ratio→DR logistic ordinates are used verbatim. Primary structural aggregate `$1,215,627/MW` (61.77% TIV); all-5 envelope `$1,275,678/MW` (64.82% TIV). Value ladder = NREL CWER 2024 (installed TIV `$1,968,000/MW`; physical replaceable `$1,623,000/MW`).
...
## v0.7 addition — solar_strong_wind pair dossier (utility-scale PV; straight-line convective wind)

The package now includes its fifth hazard×asset pair dossier and its second solar-wind-family pair: [`01_pairs/solar_strong_wind`](01_pairs/solar_strong_wind/README.md), plus a value/damage crosswalk at [`02_crosswalks/solar_strong_wind_value_damage_crosswalk.md`](02_crosswalks/solar_strong_wind_value_damage_crosswalk.md).

The crosswalk is `RESOLVED_FROM_DAMAGE_MODELING`: the **canonical `strong_wind_solar` v1.0 cell exists** in [`Damage_Modeling`](https://github.com/aamani-ai/Damage_Modeling) (`docs/cells/strong_wind_solar/current/`, `canonical_runtime_artifact: true`), so the **5 named SWS_* failure units** (`SWS_TRACKER_STRUCT`, `SWS_RACKING_STRUCT`, `SWS_MODULE_ATTACH`, `SWS_FOUNDATION_UPLIFT`, `SWS_SCADA_EXPOSED`), their value shares, and their thresholded-logistic demand→DR ordinates on the `SWS_GUST_3S_ARRAY_HEIGHT` axis (`R_eff = (V_3s/V_design)^2`) are used verbatim. The solar value ladder is `installed_TIV = $1,120,000/MWdc` (exact-confirmed by NREL/DOE Q1-2024 `$1.12/W-dc`), `physical_replaceable = $877,796/MWdc` (78.4%).

**Central finding:** no public source cleanly isolates pure straight-line / convective-wind $ severity for utility-scale solar. Every open dollar figure is hail-stow-framed (AXIS `$150k–$380k/MW` stow scenarios), hail-dominant (GCube: hail = 54% of solar claim cost), tornado-parametric (Descartes), generic all-property SCS (Gallagher Re/Aon/Swiss Re), or gated (Verisk/AIR, Moody's RMS, VDE/Cirrus, Renew Risk). The strongest OPEN solar-wind evidence is engineering/screening grade: stow-behavior severity swing, CPP aeroelastic galloping onset (`<~40 m/s`), PVEL load ratings (tracker `1800 Pa` vs fixed `2400 Pa`), and Cedar Rapids 2020 derecho forensics. 6 of 30 benchmark rows normalize to `$/MW`, all blended.

### v0.7 addition — solar_strong_wind v2 PROPOSED / NON-CANONICAL preview (does not alter the v0.7 resolved row)

A staged, **PROPOSED / NON-CANONICAL** preview of the `strong_wind_solar` v2.0 two-architecture screening candidate has been added at [`02_crosswalks/solar_strong_wind_v2_proposed_value_damage_crosswalk.md`](02_crosswalks/solar_strong_wind_v2_proposed_value_damage_crosswalk.md). It routes a single straight-line-convective hazard input by `array_architecture` into two independently-governed pathways — `fixed_tilt_ground_mount_screening_v1` (net-pressure demand axis) and `single_axis_tracker_qualified_screening_v1` (gust/Ucrit axis) — each an ordered-damage-state lognormal with a lower/central/upper **epistemic envelope** (not percentiles). It is `canonical_runtime_artifact: false`, `promotion: not_performed`; the v1.0 cell remains the canonical runtime.
...
| `solar_strong_wind` | **built in v0.7** (`RESOLVED_FROM_DAMAGE_MODELING`) | second solar-wind-family pair; utility-scale ground-mount PV under straight-line / convective wind (derecho, microburst, gust-front), tornado and hurricane/TC quarantined; crosswalk cross-mapped onto the canonical, PROMOTED `strong_wind_solar` v1.0 cell (5 SWS_* failure units + thresholded-logistic demand→DR curve on the `SWS_GUST_3S_ARRAY_HEIGHT` axis) in Damage_Modeling; only 6 of 30 benchmark rows normalize to $/MW and all are blended — no open source isolates pure straight-line-wind $ severity for solar (all hail-stow-framed, hail-dominant, tornado-parametric, generic-SCS, or gated); TIV `$1,120,000/MWdc` exact-confirmed by NREL/DOE Q1-2024 |
| `solar_strong_wind` — v2 preview | **added in v0.7** (`PROPOSED_NONCANONICAL_FROM_DAMAGE_MODELING_V2`, non-canonical) | staged, non-promoted `strong_wind_solar` v2.0 two-architecture screening candidate previewed alongside the v0.7 resolved row, without altering it; routes one straight-line-convective input by `array_architecture` into `fixed_tilt_ground_mount` (net-pressure axis) + `single_axis_tracker_qualified` (gust/Ucrit axis) pathways, ordered-damage-state lognormal with a lower/central/upper epistemic envelope; `canonical_runtime_artifact=false`, `promotion: not_performed`; **v1.0 remains the canonical `Damage_Modeling` runtime** |
...
| `solar_strong_wind` | **BUILT v0.7** — AXIS stow $/MW (`$150k–$380k`, blended), GCube hail-dominant cost, RETC high-wind frequency | **YELLOW** — Renew Risk U.S. SCS model, VDE/Cirrus severe convective / named windstorm, Verisk SCS (all gated) | **GREEN** — Cedar Rapids 2020 derecho, Oakey, Soltec, Prescott AZ forensic case set | **YELLOW** — wind/hail co-probability and storm-response triggers | **GREEN** — CPP galloping, NEXTracker stow, PVEL loads, FEMP/NREL storm-resilience | **BUILT v0.7 `RESOLVED_FROM_DAMAGE_MODELING`; no open source isolates pure straight-line-wind $ severity** |
...
| `solar_wildfire_physical` | **YELLOW** — kWh 2026 says fire is second-largest PV loss driver, but many fires are equipment-driven brushfires, not regional wildfire | **YELLOW** — VDE/Cirrus wildfire PML; general wildfire cat models | **ORANGE** — site fire case studies possible; open physical wildfire losses sparse | **YELLOW** — parametric wildfire/smoke products possible | **GREEN** — DOE/FEMP wildfire PV guidance | **Moderate for fire, weaker for offsite wildfire burnover** |
...
**Status:** good source path for fire and smoke revenue loss; weaker for pure offsite wildfire physical burnover.
