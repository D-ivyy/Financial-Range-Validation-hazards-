...
| `wind_convective_wind` (onshore turbines, tornado + straight-line/derecho) | **RED/YELLOW** — no open per-turbine convective-wind $/MW severity; Flat Ridge 2 tornado dollars are farm-level without a struck-turbine count | **YELLOW** — RMS/Moody's SCS, GCube; wind PML trend is hurricane-contaminated (`HURRICANE_TC` quarantine) | **YELLOW/ORANGE** — Flat Ridge 2 litigation, Greenfield EF4, plus a measured-null derecho | **YELLOW** — wind-speed parametric possible, not turbine-specific | **GREEN** — IEC 61400-1 class Ve50, Bouchard & Romanic fragility, DOE/NREL/EPRI forensic | **Dossier built in v0.5 (`01_pairs/wind_convective_wind`), CONVECTIVE-only; crosswalk `RESOLVED_FROM_DAMAGE_MODELING` against canonical, PROMOTED `wind_tornado_wind` cell (5 WT_* structural failure units + real speed-ratio→DR curve); 2/27 rows normalize to $/MW; hurricane/TC and hail-SCS quarantined** |
...
> **v0.7 dossier note (STRAIGHT-LINE / CONVECTIVE-wind scope).** The `solar_strong_wind` pair dossier is now built at [`../01_pairs/solar_strong_wind/README.md`](../01_pairs/solar_strong_wind/README.md), scoped to utility-scale ground-mount PV under **straight-line / convective wind** — derecho, microburst, gust-front outflow — on the 3-second-gust-at-array-height axis `SWS_GUST_3S_ARRAY_HEIGHT` (`R_eff = (V_3s/V_design)^2`). **Tornado and hurricane/TC are named-and-quarantined.** The crosswalk is `RESOLVED_FROM_DAMAGE_MODELING` against the canonical, PROMOTED `strong_wind_solar` v1.0 cell (5 SWS_* failure units — `SWS_TRACKER_STRUCT`, `SWS_RACKING_STRUCT`, `SWS_MODULE_ATTACH`, `SWS_FOUNDATION_UPLIFT`, `SWS_SCADA_EXPOSED` — + thresholded-logistic demand→DR curve). **Central finding: no open source isolates pure straight-line-wind $ severity for solar** — the strongest open dollars are AXIS hail-stow-framed `$/MW` (`$150k–$380k/MW`, blended); TIV `$1,120,000/MWdc` is exact-confirmed by NREL/DOE Q1-2024 (`$1.12/W-dc`). Only 6 of 30 benchmark rows normalize to $/MW, all blended. A staged, non-canonical two-architecture v2 preview accompanies it. Companion crosswalk at [`../02_crosswalks/solar_strong_wind_value_damage_crosswalk.md`](../02_crosswalks/solar_strong_wind_value_damage_crosswalk.md).

**Status:** built in v0.7 (`RESOLVED_FROM_DAMAGE_MODELING`); source path credible but no open source isolates pure straight-line-wind $ severity — strongest open dollars are hail-stow-blended.
...
> **v0.5 dossier note (CONVECTIVE-only scope).** The `wind_convective_wind` pair dossier is now built at [`../01_pairs/wind_convective_wind/README.md`](../01_pairs/wind_convective_wind/README.md), scoped to onshore wind turbines under **CONVECTIVE wind only** — tornado [T] and straight-line/derecho [W] — on the dimensionless speed-ratio axis `X_SPEED_RATIO_TO_IEC` (`r = V_3s_hub / Ve50_class`). Tornado = D50-shifted pathway + `0.10` core-exposure fraction; straight-line/derecho = SLW (D50-baseline) pathway of the **same** canonical curves (no separate strong-wind-for-wind cell exists upstream). **Hurricane/TC and hail-dominant SCS are named-and-quarantined** (`HURRICANE_TC`, `HAIL_CONTAMINATED_SCS`). The crosswalk is `RESOLVED_FROM_DAMAGE_MODELING` against the canonical, PROMOTED `wind_tornado_wind` cell (5 WT_* structural failure units + real speed-ratio→DR logistic curve). Only 2 of 27 benchmark rows normalize to $/MW: the strongest open dollars (Flat Ridge 2 tornado `$7.25M` net) are farm-level with no struck-turbine count (withheld), and the best straight-line anchor is a **measured null** (2020 Midwest derecho, ~126 mph gust, machines feathered). Companion crosswalk at [`../02_crosswalks/wind_convective_wind_value_damage_crosswalk.md`](../02_crosswalks/wind_convective_wind_value_damage_crosswalk.md).

> **v0.6 PROPOSED / NON-CANONICAL v2 preview.** A staged v2.0 (docs r1) candidate for the `wind_tornado_wind` cell has been previewed in parallel — it splits the single shared curve family into two independently governed pathways, `straight_line_convective` and `tornado_direct_hit`, both evaluated as mutually-exclusive ordered damage states on a single `$1,090,000/MW` turbine-equipment-assembly atom (rather than v1.0's independent 5-unit sum on the `$1,623,000/MW` physical base). The candidate is explicitly `canonical_runtime_artifact: false`, `promotion_status: proposed`, `lifecycle_state: candidate`, and `promotion: not_performed` — it is deliberately absent from the artifact index and portable library v2.5. **The cell's `current_pin` remains `wind_tornado_wind@model_v1_0__docs_r4`; v1.0 is still the canonical `Damage_Modeling` runtime**, and the `RESOLVED_FROM_DAMAGE_MODELING` status above is unchanged. See the preview at [`../02_crosswalks/wind_convective_wind_v2_proposed_value_damage_crosswalk.md`](../02_crosswalks/wind_convective_wind_v2_proposed_value_damage_crosswalk.md).
